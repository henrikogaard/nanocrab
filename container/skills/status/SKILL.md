...
node -e "import('playwright').then(() => console.log('playwright: available')).catch(() => console.log('playwright: not installed'))" 2>/dev/null
...
```
Call mcp__nanocrab__list_tasks to get scheduled tasks.
```
