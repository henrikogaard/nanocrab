...
export async function sendScheduledTaskWebhook(
  input: { url: string; taskId: string; result: string; approvalId?: string },
  fetchImpl: typeof fetch = fetch,
): Promise<WebhookDeliveryResult> {
  if (!input.url || !/^https?:\/\//i.test(input.url)) {
    throw new Error('Webhook delivery is missing a valid URL');
  }
...
export async function executeWebhookDeliveryApproval(
  ...
  return sendScheduledTaskWebhook(
    { url, taskId, result, approvalId: approval.id },
    fetchImpl,
  );
}
