...
export class TelegramChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    this.bot.on('message:voice', (ctx) => {
      ...
      this.downloadFile(fileId, group.folder, filename).then(async (result) => {
        let content: string;
        if (result) {
          const transcript = await transcribeAudio(result.hostPath);
          if (transcript) {
            content = `[Voice: "${transcript}"] (${result.containerPath})`;
          } else {
            content = `[Voice message] (${result.containerPath})`;
          }
        } else {
