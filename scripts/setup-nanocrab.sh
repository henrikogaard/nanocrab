...
# ── State tracking ──
load_state() {
  COMPLETED=$(node -e "const fs=require('fs'); const [statePath,...entries]=process.argv.slice(1); const names=entries.map((entry)=>entry.split(':')[0]); let raw={}; let sourceExists=fs.existsSync(statePath); try{raw=JSON.parse(fs.readFileSync(statePath,'utf8'));}catch{} let changed=sourceExists && (raw.version!==2 || !raw.steps || Array.isArray(raw.completed)); let state={version:2,updatedAt:new Date().toISOString(),steps:Object.fromEntries(names.map((name)=>[name,{status:'pending'}]))}; if(Array.isArray(raw.completed)){for(const name of raw.completed) if(state.steps[name]) state.steps[name]={status:'completed'};} if(raw.steps){for(const name of names){if(raw.steps[name]) state.steps[name]=raw.steps[name]; else changed=true;}} for(const name of names){if(state.steps[name]?.status==='running'){state.steps[name]={...state.steps[name],status:'failed',failedAt:new Date().toISOString(),error:'Setup was interrupted while this step was running'}; changed=true;}} if(changed) fs.writeFileSync(statePath, JSON.stringify(state,null,2)+'\n', {mode:0o600}); console.log(names.filter((name)=>state.steps[name]?.status==='completed').join(','));" "$STATE_FILE" "${STEPS[@]}" 2>/dev/null || echo "")
