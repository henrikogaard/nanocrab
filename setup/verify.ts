...
import { getCodexAuthStatus } from '../src/codex-auth.js';
...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.nanocrab')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.nanocrab'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active nanocrab`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
  ...
  if (fs.existsSync(envFile)) {
    ...
    const envVars = readEnvFile(['DEFAULT_PROVIDER']);
    const codexAuth = getCodexAuthStatus();
    if (
      /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY)=/m.test(envContent) ||
      (envVars.DEFAULT_PROVIDER === 'codex' && codexAuth.configured)
    ) {
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'nanocrab', 'mount-allowlist.json'),
    )
  ) {
