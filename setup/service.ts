...
<dict>
    ...
    <string>${projectRoot}/logs/nanocrab.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/nanocrab.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.nanocrab');
  } catch {
...
/**
 * Kill any orphaned nanocrab node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned nanocrab processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/nanocrab.service';
    ...
    unitPath = path.join(unitDir, 'nanocrab.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=NanoCrab Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/nanocrab.log
StandardError=append:${projectRoot}/logs/nanocrab.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned nanocrab processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable nanocrab`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start nanocrab`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active nanocrab`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-nanocrab.sh');
  const pidFile = path.join(projectRoot, 'nanocrab.pid');

  const lines = [
    '#!/bin/bash',
    '# start-nanocrab.sh — Start NanoCrab without systemd',
    ...
    '    echo "Stopping existing NanoCrab (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NanoCrab..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/nanocrab.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/nanocrab.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NanoCrab started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/nanocrab.log"`,
  ];
