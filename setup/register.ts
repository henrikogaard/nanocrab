...
export async function run(args: string[]): Promise<void> {
  ...
  // The agent runs with CWD=/workspace/group and reads AGENTS.md from there.
  ...
  if (copyAgentInstructionsTemplate(templateDir, groupDir)) {
    logger.info(
      { file: path.join(groupDir, AGENT_INSTRUCTIONS_FILE), template: templateDir },
  ...
  if (parsed.assistantName !== 'Andy') {
    ...
    const groupDirs = fs
      ...
      .map((d) => path.join(groupsDir, d))
      .filter((d) => fs.statSync(d).isDirectory());

    for (const dir of groupDirs) {
      let content = readAgentInstructions(dir);
      if (!content) continue;
      ...
      writeAgentInstructions(dir, content);
      logger.info(
        { file: path.join(dir, AGENT_INSTRUCTIONS_FILE) },
        'Updated AGENTS.md',
      );
    }
