...
- **Terminal Workspace And Session History** — the owner-only Developer Hub terminal pairs Terminal or Files with Chat, Diff, Logs, or Search panes, remembers the selected layout, and persists bounded/redacted transcripts with session ownership. Live processes can reconnect while NanoCrab remains running; after a service restart the saved transcript is explicitly marked read-only because the underlying shell process cannot be restored.
