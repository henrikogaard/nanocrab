...
| Channel does not respond | Channel auth state, group registration, trigger name, message logs. If the bot shows typing but never replies, check agent-net UFW allow for credential proxy `:3001`, ensure only one NanoCrab process is running, and inspect container exit 137 / orphan cleanup loops (`docs/DEBUG_CHECKLIST.md`). |
