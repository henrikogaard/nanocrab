...
- Replace the current dev terminal experience with a session-based terminal/chat surface:
  ...
  - DONE: shared work-session state in Chat and Terminal, with explicit loading, ready, reconnecting, unavailable, and interrupted/read-only terminal states.
  - DONE: explicit Chat promotion to the existing Cowork projects or Code workspace; plain messages are never moved automatically.
