...
The default-deny claim is **proven** only after an operator runs the canary on
the target host. Until then the topology mechanism is shipped but the claim is
operator-verified, not asserted by the repository.
