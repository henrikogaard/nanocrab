...
| Provider profiles and coding runtime profiles | `#/integrations`, `#/settings`, `#/agents`     | `/api/providers`, `/api/system/provider`, `/api/system/provider/profiles`, `/api/agents/coding/runtime-profiles` | Named Runner CLI + provider + model selection for coding jobs, with compatibility and readiness validation | Ready |
...
| Container egress governance (planned)     | `#/audit`, `#/security`, policy/dry-run surfaces  | Planned: container network isolation, egress gateway, destination-bound credentials, hardening flags     | Planned: default-deny outbound network, deny/allow egress audit, red-team smoke, signed/hash-chained audit export        | Planned — tracked as GitHub Project `#7` issues after AgentPaaS review |
