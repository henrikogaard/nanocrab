...
| `/coding-pick owner/repo labels=a,b tool=codex provider=codex model=gpt-5.4 [no-pr]` | Registered channels | Authorized operators           | Picks an open issue and starts an isolated coding job with an explicit validated runtime.                                             |
